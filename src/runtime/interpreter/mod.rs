...
pub(super) fn throw_exception(vm: &mut VmThread, class_name: &str) -> VmError {
    use crate::oops::Oop;
    let reg = vm
        .registry()
...
pub(super) fn capture_backtrace(vm: &mut VmThread, exc: Reference) {
...
impl<'a> Interpreter<'a> {
    ...
    pub fn interpret_with(&self, frame: &mut Frame, vm: &mut VmThread) -> Result<Value, VmError> {
    ...
    fn run(&self, frame: &mut Frame, vm: &mut VmThread) -> Result<Value, VmError> {
        let mut pc: usize = 0;
        loop {
            if pc >= self.code.len() {
...
mod tests {
    ...
    fn assert_throws(result: Result<Value, VmError>, vm: &VmThread, expected_class: &str) {
    ...
    fn athrow_caught_jumps_to_handler() {
        ...
        let mut vm = crate::runtime::VmThread::new(std::sync::Arc::clone(&reg));
