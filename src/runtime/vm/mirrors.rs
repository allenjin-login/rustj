...
//! Module 镜像(4.14a)、按名写实例字段 helper 归此。镜像对应的共享态在 `Vm`
//!(`class_mirrors`(Class BiMap)/`module_mirrors`/`unnamed_module`)。
...
impl VmThread {
    ...
    /// 反查方向(Class BiMap `by_ref`);后续命中直接返。使 `Foo.class == Foo.class`、
