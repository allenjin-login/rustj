...
//!
//! 这是 Layer 3.3 的"能否跑通真实字节码"判据。需要 PATH 中有 `javac`(无则跳过)。
//! 方法刻意只用数值 + `invokestatic`(同类自调/递归/互调)。
...
use rustj::runtime::ClassProvider;
...
/// 只持有单个类的 [`ClassProvider`]:仅当请求名等于该类自身内部名时返回它。
/// 适用于同类自调/递归(本增量的 `invokestatic` 范围)。
struct OneClass<'a> {
    cf: &'a ClassFile,
}

impl<'a> ClassProvider for OneClass<'a> {
    fn class_by_name(&self, name: &str) -> Option<&ClassFile> {
        if self.cf.this_class_name() == Some(name) {
            Some(self.cf)
        } else {
            None
        }
...
/// 执行静态方法(支持 `invokestatic`):按实参类型与槽位约定写入局部变量,返回结果值。
fn run(cf: &ClassFile, name: &str, desc: &str, args: &[Arg]) -> Value {
    let method = find_method(cf, name, desc);
    let code = method.code.as_ref().unwrap_or_else(|| panic!("{name} 应有 Code"));
    let provider = OneClass { cf };
    ...
    let interp = Interpreter::with_classes(&code.code, &cf.constant_pool, &provider);
    interp
        .interpret(&mut frame)
...
"#;
    let class_path = compile(source, "Recursion");
    let bytes = std::fs::read(&class_path).unwrap();
    let cf = parse(&bytes).expect("解析应成功");

    // 递归 invokestatic(单 int 实参/返回)
    assert_eq!(run(&cf, "factorial", "(I)I", &[Arg::I(0)]), Value::Int(1));
    assert_eq!(run(&cf, "factorial", "(I)I", &[Arg::I(1)]), Value::Int(1));
    assert_eq!(run(&cf, "factorial", "(I)I", &[Arg::I(5)]), Value::Int(120));
    assert_eq!(run(&cf, "factorial", "(I)I", &[Arg::I(10)]), Value::Int(3_628_800));

    // 双递归(同一方法自调两次后相加)
    assert_eq!(run(&cf, "fib", "(I)I", &[Arg::I(0)]), Value::Int(0));
    assert_eq!(run(&cf, "fib", "(I)I", &[Arg::I(1)]), Value::Int(1));
    assert_eq!(run(&cf, "fib", "(I)I", &[Arg::I(10)]), Value::Int(55));
    assert_eq!(run(&cf, "fib", "(I)I", &[Arg::I(15)]), Value::Int(610));

    // 互调:sumOfSquares 调 square(两个 int 实参)
    assert_eq!(run(&cf, "sumOfSquares", "(II)I", &[Arg::I(3), Arg::I(4)]), Value::Int(25));
    assert_eq!(run(&cf, "sumOfSquares", "(II)I", &[Arg::I(5), Arg::I(12)]), Value::Int(169));

    // 嵌套调用作为实参 + 多层递归(Ackermann:验证调用栈深度与返回值回填)
    assert_eq!(run(&cf, "ackermann", "(II)I", &[Arg::I(2), Arg::I(3)]), Value::Int(9));
    assert_eq!(run(&cf, "ackermann", "(II)I", &[Arg::I(3), Arg::I(3)]), Value::Int(61));
}
...
"#;
    let class_path = compile(source, "Mixed");
    let bytes = std::fs::read(&class_path).unwrap();
    let cf = parse(&bytes).expect("解析应成功");

    // pow(2, 10) = 1024:long 在 slot 0-1,int 在 slot 2
    assert_eq!(run(&cf, "pow", "(JI)J", &[Arg::L(2), Arg::I(10)]), Value::Long(1024));
    assert_eq!(run(&cf, "pow", "(JI)J", &[Arg::L(3), Arg::I(5)]), Value::Long(243));
    assert_eq!(run(&cf, "pow", "(JI)J", &[Arg::L(5), Arg::I(0)]), Value::Long(1));

    // shift(3, 100) = 100 + 100 + 100 + 100 = 400:int 在 slot 0,long 在 slot 1-2
    assert_eq!(run(&cf, "shift", "(IJ)J", &[Arg::I(3), Arg::L(100)]), Value::Long(400));

    // powDouble(2.0, 10) = 1024.0:double(cat-2)实参 + cat-2 返回
    match run(&cf, "powDouble", "(DI)D", &[Arg::D(2.0), Arg::I(10)]) {
        Value::Double(v) => assert!((v - 1024.0).abs() < 1e-9),
        other => panic!("powDouble 返回非 double:{other:?}"),
    }

    // powFloat(2.0, 5) = 32.0:float(cat-1)实参 + cat-1 返回
    match run(&cf, "powFloat", "(FI)F", &[Arg::F(2.0), Arg::I(5)]) {
